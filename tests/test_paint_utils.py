...
import app.paint.utils as utils



class PaintUtilsTestCase(unittest.TestCase):
    def setUp(self):
    ...
    def test_get_figure_tag(self):
        #with self.app_context, self.app.test_request_context():
        #    figure_tag = utils.get_figure_tag(x, y)
        figure_tag = utils.get_figure_tag(x, y)
        self.assertTrue('<g class="deletable">' in figure_tag)

    def test_get_bezier_tag(self):
        c = np.random.randint(15, size=(4, 2))
        bezier_tag = utils.get_bezier_tag(c)
        self.assertTrue('<g class="deletable">' in bezier_tag)

    def test_get_triangle_tag(self):
        c = np.random.randint(15, size=(4, 2))
        triangle_tag = utils.get_triangle_tag(c)
        self.assertTrue('<g class="deletable">' in triangle_tag)

    def test_get_rectangle_tag(self):
        c = np.random.randint(15, size=(4, 2))
        rectangle_tag = utils.get_rectangle_tag(c)
        self.assertTrue('<g class="deletable">' in rectangle_tag)

    def test_get_ellipse_tag(self):
        c = np.random.randint(15, size=(4, 2))
        ellipse_tag = utils.get_ellipse_tag(c)
        self.assertTrue('<g class="deletable">' in ellipse_tag)
    ...
    def test_scale_back_coordinates(self):
        x_scaled = np.array([ 2.5, 15. , 27.5])
        y_scaled = np.array([12.5, 15. , 17.5])
        scale = 4
        x_scaled_med = 12.5
        y_scaled_med = 52.5
        x_len = 100
